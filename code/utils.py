...
def get_pulsars_ipta(psrlist=None, ephem='DE436', use_cache=True):
    """
    ...
    """
    DATADIR = "$HOME/projects/working/DR2/release/VersionB/"
    
    if psrlist is None:
        psrlist = np.loadtxt('psrlist-ipta.txt', dtype=np.unicode_)
    ...
    if os.path.exists(cached_file) and use_cache:
        ...
        for pulsar in psrlist:
            print("Add pulsar" + pulsar)
            par = DATADIR + pulsar + "/" + pulsar + ".IPTADR2.par"
            tim = DATADIR + pulsar + "/" + pulsar + ".IPTADR2.tim"
...
#     logAmin = -5
    logAmin = np.percentile(samples, q=1)
    logAmax = 0 

    prior = 1 / (logAmax - logAmin)
    #dA = np.linspace(0.01, 0.1, 100)
    dA = np.linspace(0.01, 0.1, ntol//2)
    bf = []
    bf_err = []
